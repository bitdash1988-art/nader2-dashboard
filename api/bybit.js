...
export default async function handler(req) {
  ...
  if ((process.env.BYBIT_DEMO || "").toLowerCase() !== "true") return json({ ok: false, error: "DEMO_GUARD_BLOCKED" }, 403);
  ...
  try {
    ...
    if (action === "health") return json({ ok: true, mode: "DEMO", base_url: BASE, demo_guard: true, server_time: await serverTime() });

    const key = process.env.BYBIT_API_KEY || "";
    const secret = process.env.BYBIT_API_SECRET || "";
    if (!key || !secret) return json({ ok: false, error: "BYBIT_SECRETS_MISSING" }, 500);

    if (action === "balance") return json({ mode: "DEMO", ...(await privateGet("/v5/account/wallet-balance", { accountType: "UNIFIED" }, key, secret)) });
    ...
    if (action === "ticker") {
      ...
      const r = await fetch(`${BASE}/v5/market/tickers?category=${encodeURIComponent(category)}&symbol=${encodeURIComponent(symbol)}`, { cache: "no-store" });
    ...
    if (action === "order_status") {
      const category = String(body.category || "spot");
      const p = { category };
      if (body.orderId) p.orderId = String(body.orderId); else if (body.orderLinkId) p.orderLinkId = String(body.orderLinkId); else return json({ ok: false, error: "ORDER_ID_REQUIRED" }, 400);
      return json({ mode: "DEMO", ...(await privateGet("/v5/order/realtime", p, key, secret)) });
    }

    if (action === "cancel_demo_order") {
      const p = { category: String(body.category || "spot"), symbol: String(body.symbol || "BTCUSDT").toUpperCase() };
      if (body.orderId) p.orderId = String(body.orderId); else if (body.orderLinkId) p.orderLinkId = String(body.orderLinkId); else return json({ ok: false, error: "ORDER_ID_REQUIRED" }, 400);
